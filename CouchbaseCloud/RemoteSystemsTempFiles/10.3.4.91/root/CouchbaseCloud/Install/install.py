...
def handleNewDeployment(result,depReq):            
        ...
        while  op.find(substr) != -1 :
             p2 = subprocess.Popen(r'sudo -s python scripts/install.py -i /tmp/{0} -p product=cb,version=3.0.0-1156-rel'.format(result['testmkfile']),
...
def handleNewInstance(result):
    ...
    while  op.find(substr) != -1 :
         p2 = subprocess.Popen(r'sudo -s python scripts/install.py -i /tmp/{0} -p product=cb,version=3.0.0-966-rel,amazon=true'.format(result['testmkfile']),
                      cwd = r'/auth/testrunner', shell =True, stdout = subprocess.PIPE)
    ...
    dns = (cb.get("{0}".format(result['vmprimary'])).value)['dns']
    
    
    username = result['username']
    depname = result['depname']
    
    depIndex = (cb.get("{0}".format(result['sessiondetails'])).value)['deploymentIndex']
