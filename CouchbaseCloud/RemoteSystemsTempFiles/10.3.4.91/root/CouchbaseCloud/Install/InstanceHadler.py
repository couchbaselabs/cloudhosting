...
def handleNewInstances(request,cb,depReq):
    ...
    resultSession = cb.get("{0}".format(request['sessiondetails'])).value
...
def delInstance(request,cb,depReq):
    ...
    pvm = cb.get("{0}".format(request['vmprimary'])).value
    print list

     
    sessRes = cb.get("{0}".format(request['sessiondetails'])).value
...
def aws_mkfile_add(vm, user, key):
    ...
    filekey.close()
    '''
    file = "/tmp/test-{0}.ini".format(tm = timestamp())
    cfgfile = open(file,'w')
